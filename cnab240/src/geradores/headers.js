...
export function geraDetalheA(objeto) {
  ...
  //Caso seja PIX
  if (
    objeto_temp.cdg_camara_favorecido == "009"
  ) {
    return `${geraControle({ ...objeto_temp, cdg_registro: 1 })}${
      geraServico(objeto_temp)
    }${geraFavorecido(objeto_temp)}${geraCredito(objeto_temp)}${
      geraDetalhePix(
        objeto_temp.num_doc,
      ...
      preencheEspaco("", 2)
    }${preencheEspaco(objeto_temp.cdg_finalidade_doc_ted), 5}${
      preencheEspaco("", 3)
    }${objeto_temp.aviso_favorecido}${preencheEspaco, 10}`;
  }
}
